#!/usr/bin/env bash
#
# mp4san/mayhem/test.sh — RUN signalapp/mp4san's own test suite (`cargo test`) and emit a CTRF
# summary. exit 0 iff no test failed.
#
# PATCH-grade oracle: mp4san ships an extensive in-crate unit-test suite (the box parsers,
# bitstream readers, ftyp/moov/stbl handling, etc.) that asserts concrete parse results and
# round-trip behavior via assert_matches!/assert_eq! — so a no-op / "exit(0)" / output-altering
# patch CANNOT pass. This script only RUNS the suite via `cargo test`; it never builds fuzz
# targets.
#
# We run with the crate's DEFAULT features (the ffmpeg/gpac differential-verifier features are
# OPT-IN and require system libraries, so they stay off): the integration test `tests/test-data.rs`
# iterates the `test-data` git submodule, which is intentionally not fetched (update = none) — its
# helper returns gracefully on a missing dir, and the ffmpeg/gpac asserts are no-ops without their
# features. The hundreds of self-contained unit tests are the real oracle.
set -uo pipefail
...
echo "=== running cargo test (mp4san workspace, default features) ==="
